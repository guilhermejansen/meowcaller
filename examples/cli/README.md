# cli — meowcaller calling-layer demo
...
```
cli loopback            Mic → MLow encode → E2E-SRTP protect (RTP WARP header +
...
cli call <target>       Log in (QR on first run), resolve the peer LID, discover
...
cli autoaccept          Log in and auto-accept incoming calls: decrypt the offer's
