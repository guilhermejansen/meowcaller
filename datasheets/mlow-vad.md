...
Voice-activity detector: per-internal-frame speech-activity probability and the
packet-level `coded_as_active_voice` flag the bitrate controller and the
voiced/unvoiced classifier read. Media layer (encoder front-end); runs on the raw
int16 input PCM at 16 kHz, 320 samples per internal frame.

**Validation vector:** `vad_ground_truth.json` — the C `enc_dump` of
`smpl_VAD_GetSA_Q8_c` on the committed `mic_clip.raw`, per packet/frame (`spact`
...
//! `coded_as_active_voice` flag the bitrate controller and voiced/unvoiced classifier read. Faithful
//! fixed-point port of `smpl_VAD_GetSA_Q8_c` + `smpl_VAD_GetNoiseLevels` + the 2-band allpass
//! filterbank + the per-packet hangover. Runs on the raw int16 input PCM (no encoder HP), 320 samples
//! per internal frame at 16 kHz.

// ---- SILK fixed-point primitives (silk/macros.h, SigProc_FIX.h, Inlines.h) ----
...
fn lin2log(in_lin: i32) -> i32 {
    ...
    // silk_SMLAWB(frac_Q7, frac_Q7*(128-frac_Q7), 179) + ((31-lz) << 7)
...
/// Persistent SILK VAD state (`smpl_VAD_state`), carried across packets.
...
impl SmplVadState {
    ...
    /// First-order ARMA HP filter with zero at DC (`smpl_filt_hp_FIX`), in place over `len` samples.
    ...
    /// 2-band split via first-order allpass filters (`smpl_ana_filt_bank_1`). Writes low band to
    /// `out_l[0..N/2]` and high band to `out_h[0..N/2]`. `s` is the carried 2-element state.
    fn ana_filt_bank_1(
        inp: &[i32],
        s: &mut [i32; 2],
        out_l: &mut [i32],
    ...
    /// smpl_VAD_GetSA_Q8_c: returns speech_activity_Q8 for one `framelen`-sample int16 frame.
    ...
    /// X[hi_off..hi_off+n/2]. Mirrors the C aliasing (input and low-out overlap; the C reads
    /// `in[2k]/in[2k+1]` before writing `out[k]`, so a forward scan is safe since k <= 2k).
...
mod ground_truth_tests {
    ...
    // The SILK VAD is a fixed-point port, so its per-frame speech-activity probability and the
    // packet-level coded_as_active_voice flag must match the reference libopus encoder bit-exactly.
    // `vad_ground_truth.json` is the C enc_dump (smpl_VAD_GetSA_Q8_c) on the committed mic_clip.raw,
    // over the packets where the carried noise-level state stays bit-exact.
    #[test]
    fn vad_matches_c_ground_truth() {
        let raw = include_bytes!("testdata/mic_clip.raw");
        ...
        for rec in gt {
            ...
            assert!(
                ...
                "pkt {pkt} frame {frame}: spact {spact} != C {c_spact}"
            );
...
type SmplVadState struct {
	// allpass filterbank state (3 stages x 2), per-band carried subframe energy, noise levels,
	// inverse noise levels, noise-level bias, counter, HP filter state, tuning params, DTX hangover.
}

// smpl_VAD_Init.
func NewSmplVadState() *SmplVadState

// VAD output for one 60 ms packet.
type VadPacketResult struct {
	VadResults         [3]float32 // per-internal-frame speech-activity probability
	CodedAsActiveVoice bool
}

// Process one 60 ms packet (3 internal frames of framelen int16 samples).
func (s *SmplVadState) ProcessPacket(pcmI16 []int16, framelen int) VadPacketResult
```

## Implementation suggestions (guidance, not authoritative)

- This is a fixed-point port and the validation is bit-exact (the vector asserts
  `spact` to `< 1e-4` and `cav` exactly), so every SILK macro must reproduce the
  exact integer arithmetic. `i32` → Go `int32`, `i64` intermediates → `int64`,
  `i16` casts → `int16`. The `(b as i16 as i64)` pattern is sign-truncate-to-16
  then widen: in Go, `int64(int16(b))`.
- The saturating/wrapping macros are load-bearing: `smulbb`/`smlabb` use
  `wrapping_mul`/`wrapping_add` (Go int32 overflow already wraps — use plain `*`/`+`
  on `int32`). `add_pos_sat32` tests the sign bit of the `u32` sum
  (`(uint32(a)+uint32(b)) & 0x80000000`) — keep the unsigned arithmetic.
- `clz32` is count-leading-zeros returning 32 for input 0: use
  `bits.LeadingZeros32(uint32(x))`. `ror32` is a 32-bit rotate-right with
  `rot & 31`: use `bits.RotateLeft32(uint32(x), -(rot & 31))` or the equivalent
  right rotate; the reference folds negative rotates via the mask, so mask first.
- `lin2log`, `sqrt_approx`, `sigm_q15` are the SILK approximations, NOT real
  `log2`/`sqrt`/`sigmoid`. Do not substitute `math.*` — the bit-exact output depends
  on these integer approximations and their lookup tables (`SLOPE`/`POS`/`NEG`).
- `div32` is C integer division (truncates toward zero). Go `/` on `int32` matches.
  Watch the `nl[b]` denominators: the reference does not guard against zero because
  the init guarantees `nl[b] >= 100`; preserve the init so the divisor stays
  positive.
- The 2-band split has two forms: one writing to disjoint slices
  (`ana_filt_bank_1`) and one in-place with overlapping input/low-output
  (`ana_filt_bank_1_inplace`). The in-place form relies on reading `in[2k]`/`in[2k+1]`
  before writing `out[k]` (safe because `k <= 2k`). Keep the forward scan and the
  read-before-write ordering.
- `0xFF80_0000u32 as i32` is a bit-mask constant reinterpreted as a signed int32;
  in Go write `int32(0xFF800000)` (or `^int32(0x007FFFFF)`) — it is a top-9-bits
  test, not an arithmetic value.
- `SPEECH_ACTIVITY_DTX_THRES_Q8` is `(0.05 * 256.0) as i32 == 12`; the float-to-int
  cast truncates. `TODO(human):` either compute it the same way or hardcode `12`
  with a comment, and confirm the hangover decrement `PACKET_MS / FRAMES_PER_PACKET`
  (== `60/3` == `20`) matches the C.
