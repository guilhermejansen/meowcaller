...
# Datasheet: `mlow/pulse`

Decodes the excitation pulse blocks of one internal frame from the range-coded
bitstream: the total pulse count, the recursive split across subframes, the
per-position magnitudes, and the signs. Media layer (codec decode pipeline).
...
```rust
//! MLow PVQ-style pulse decode (post-LSF blocks of WASM func 3545). Ported from the Go reference
//! (`smpl_pulse.go`). The active 0x50 capture frames are config=0 (p6=0), so the pulse COUNT uses
//! the NB triangular prior; the split/magnitude/sign machinery replicates the WASM pointer
//! arithmetic against the embedded heap window (`smpl_mem.rs`) so the bitstream is consumed
//! bit-for-bit. `wrapping_*` mirrors the WASM's i32/u32 modular arithmetic.
...
pub(crate) fn decode_smpl_pulses(
    ...
    if p6 != 0 {
        ...
        let cdfp = mem.u32(ent.wrapping_add(0x11d8));
        let n = mem.u32(ent.wrapping_add(0x11dc)) as usize;
        let cdf = mem.cdf_at(cdfp, n);
    ...
    if total != 0 {
        ...
        if sum < hi_bound {
            ...
            let off = cdfp
                .wrapping_add((sum as u32) * 2)
                .wrapping_sub((lo as u32) * 2);
            let n = ((hi_bound - sum) + 2) as usize;
            let cdf = mem.cdf_at(off, n);
            sum += dec.decode_cdf(&cdf);
        }
        if sum > 0 {
            let s0 = smpl_split_3537(dec, mem, sum, subfr_len16, g_cc.wrapping_add(0xcd8));
            split[0] = s0;
            split[1] = sum - s0;
        }
        if sum < total {
            let s2 = smpl_split_3537(dec, mem, total - sum, subfr_len16, g_cc.wrapping_add(0xcd8));
            split[2] = s2;
...
mod tests {
    ...
    // counts + the full signed pulse vector against the Go reference.
    #[test]
    fn pulses_match_go() {
        ...
        for rec in arr {
            ...
            let pr = decode_smpl_pulses(&mut dec, mem, 320, 4, 1, 0, lsf.stage1);
...
- The `wrapping_*` calls are load-bearing: every address/count add/mul/shift is
  `u32`/`i32` modular arithmetic mirroring the WASM. In Go this is the default
  overflow behavior for fixed-width integer types, so use `uint32`/`int32` and plain
  operators; do NOT widen to `int`/`uint` where the source wraps.
- `tri_t` mixes `wrapping_add`/`wrapping_mul`/`wrapping_sub` on `u32` then masks
  `& 0xffff`; keep every intermediate a `uint32` so the truncation matches.
- `c_base_off` is read as `i64` and the `(c_base_off - pos as i64) * 2` offset is
  cast back to `u32`; preserve the `int64` intermediate then convert to `uint32`.
- The sign block extracts the MSB with `((bitfield >> 14) & 2) - 1` yielding +1/-1;
  keep `bitfield` a `uint32` and the shift `wrapping_shl` (Go `<<` on `uint32`).
- `pos_list`/`mag_list` grow dynamically → Go slices with `append`; `pulses` is a
  fixed-length `[]int32` of length `p2`.
- This module depends on a `RangeDecoder` (decode/update/decode_cdf/decode_raw_symbol)
  and a `SmplMem` heap window (u32/u8/cdf_at). `TODO(human):` confirm those two
  surfaces are landed first; the decode is meaningless without them.
