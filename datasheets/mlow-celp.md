...
# Datasheet: `mlow/celp` (encoder-side CELP excitation)

The closed-loop CELP excitation encoder used by `mlow/encoder`'s analysis front-end
(`smpl_analyze_frame_st`): per subframe it builds the perceptually-weighted impulse
response, runs the ACB/LTP search (voiced) and the FCB pulse search, quantizes the
gains, and returns the chosen pulses + indices + the reconstructed LPC excitation
(`CelpSubframeOut`). Ported from `smpl_celp.rs` (Meta's `smpl_celp_enc.c`). This is the
encode-side counterpart of `mlow/synth`'s `smpl_celpdec`.

**Validation:** no isolated byte-exact vector exists for the CELP encoder. It is
exercised by the reference's `encode_*_runs` smoke tests (shape/range invariants) and,
...
impl FcbState {
    fn new() -> Self {
        FcbState {
            pulse_positions: vec![0; SMPL_MAX_SF_LEN],
            pulse_signs: vec![0.0; SMPL_MAX_SF_LEN],
            num: vec![0.0; SMPL_MAX_SF_LEN],
            den: vec![0.0; SMPL_MAX_SF_LEN],
        }
    }
}
...
impl CelpEncoder {
    ...
    fn add_pulse(
        ...
        if pitch_sharp == 0.0 {
            ...
            // dd_den
            let mut dd_den = vec![0.0f32; SMPL_MAX_SF_LEN];
            g1 = 1.0f32;
            let mut pos = fcb_pos_new;
            while pos < fcb_subfrlen as i32 {
                ...
                while k < fcb_subfrlen as i32 {
                    ...
                    let mut i = start_i;
                    while i < end_i {
                        dd_den[i as usize] += g2 * self.phi_flip[(col_off + i + k) as usize];
                        i += 1;
                    }
                    g2 *= pitch_sharp;
                    k += lag;
            ...
            for i in 0..fcb_subfrlen {
                sc.fcb_states[write_idx][idx].den[i] += 2.0 * fcb_sign_new * d_sign[i] * dd_den[i];
            }
        }
        ...
        smpl_get_maxi_k(&q, &mut sort_ix, fcb_subfrlen, numsurv);
    ...
    fn smpl_fcb_search_deldec(
        ...
        let fcb_subfrlen = self.fcb_subfrlen;
        let mut sc = FcbSearchScratch::new();

        let mut d_new = vec![0.0f32; SMPL_MAX_SF_LEN];
        let mut d_abs = vec![0.0f32; SMPL_MAX_SF_LEN];
        let mut d_sign = vec![0.0f32; SMPL_MAX_SF_LEN];
        let phi0 = self.phi[0];

        if pitch_sharp != 0.0 && lag > 0 && lag < fcb_subfrlen as i32 {
            d_new[..fcb_subfrlen].copy_from_slice(&d[..fcb_subfrlen]);
            for j in 0..fcb_subfrlen {
                let mut g = pitch_sharp;
                let mut i = lag + j as i32;
                while i < fcb_subfrlen as i32 {
                    d_new[j] += g * d[i as usize];
                    g *= pitch_sharp;
                    i += lag;
                }
            }
            calc_d_abs_and_sign(&d_new, fcb_subfrlen, &mut d_abs, &mut d_sign);
        } else {
            calc_d_abs_and_sign(d, fcb_subfrlen, &mut d_abs, &mut d_sign);
            pitch_sharp = 0.0;
        }
        ...
        let mut q = vec![0.0f32; SMPL_MAX_SF_LEN];
        {
            ...
            if pitch_sharp == 0.0 {
                ...
                let (num0, den0) = {
                    let st = &sc.fcb_states[ri][0];
                    (st.num.clone(), st.den.clone())
                };
                smpl_celp_q(&num0, &den0, fcb_subfrlen, &mut q);
            }
        }

        let mut sort_ix = [0i32; SMPL_CELP_MAX_NUMSURV];
        smpl_get_maxi_k(&q, &mut sort_ix, fcb_subfrlen, surv[0] as usize);
        sc.fcbs_size = 0;
        {
            let ri = sc.read_idx;
        ...
        if fcb_pulses_max[SMPL_CELP_IDX_MAIN] > 1 {
            for pulse_nr in 2..fcb_pulses_max[SMPL_CELP_IDX_MAIN] as usize {
                ...
                for i in 0..cand_size {
                    q[i] = sc.fcb_candidates[i].wnrg;
                }
                smpl_get_maxi_k(&q, &mut sort_ix, cand_size, surv[pulse_nr - 1] as usize);
                sc.fcbs_size = 0;
...
impl CelpEncoder {
    pub(crate) fn encode_subframe(
        ...
        let mut imp_lpc_rev = vec![0.0f32; 2 * SMPL_MAX_L_RESP - 1];
        ...
        {
            let imp = self.imp_lpc_buf[SMPL_LPC_ORDER..SMPL_LPC_ORDER + l_resp].to_vec();
            let mut phi = vec![0.0f32; SMPL_MAX_SF_LEN];
            self.perc_filt_ma(&imp_lpc_rev, rev_base, l_resp, &imp, l_resp, &mut phi);
        ...
        let mut res_lpc_pad = vec![0.0f32; fcb_subfrlen + l_resp + 1];
        res_lpc_pad[..fcb_subfrlen].copy_from_slice(&res_lpc[..fcb_subfrlen]);
        let mut d_lpc = vec![0.0f32; SMPL_MAX_SF_LEN];
        {
            let c_off = SMPL_MAX_SF_LEN - l_resp + 1;
            smpl_mult_symtoepl2(
                &self.phi_flip[c_off..],
        ...
        let mut zir_lpc = vec![0.0f32; SMPL_MAX_SF_LEN];
        ...
        if !self.ignore_zir {
            ...
            {
                let imp = self.imp_lpc_buf[SMPL_LPC_ORDER..SMPL_LPC_ORDER + l_resp].to_vec();
                self.perc_filt_ma(&zir_tmp, zt, l_resp, &imp, l_resp, &mut ht_zir[ht..]);
            }
        ...
        // ACB
        let mut acb_basis = vec![0.0f32; SMPL_MAX_SF_LEN * SMPL_ACBG_M];
        let mut acb = vec![0.0f32; SMPL_MAX_SF_LEN];
        let mut d_ltp = vec![0.0f32; SMPL_MAX_SF_LEN];
        ...
        let mut wtgt_tmp = vec![0.0f32; SMPL_MAX_SF_LEN + 2 * SMPL_MAX_L_RESP - 1];
        let wt = SMPL_MAX_L_RESP - 1;
        let mut wtgt = vec![0.0f32; SMPL_MAX_SF_LEN + SMPL_MAX_L_RESP];
        ...
        let mut exc_fcb = vec![0.0f32; SMPL_MAX_SF_LEN];
        let tbl = celp_tables();
        for r in 0..SMPL_CELP_MAX_RATES {
...
// CelpSubframeOut is the per-subframe encode result (pulses + indices + reconstructed excitation).
...
// CelpEncoder is the persistent encode-side CELP state (ACB/zir history across subframes & frames).
type CelpEncoder struct { /* unexported */ }

func NewCelpEncoder(lowRate bool, percRespLen, fcbSubfrlen, subfrPerPacket int) *CelpEncoder

func (e *CelpEncoder) EncodeSubframe(
	resLpc []float32,
	predcoef *[17]float32,
	percWghtResp []float32,
	lags []float32,
	subfrImportance [smplCelpMaxRates]float32,
	fcbPulsesMax [smplCelpMaxRates]int16,
	surv []int16,
) CelpSubframeOut

// smpl_distribute_fcb_surv: split tot_surv survivors across pulse counts (numsurv out-param).
func smplDistributeFcbSurv(numsurv []int16, maxPulses, totSurv int32)

// smpl_celp_uv_gain: unvoiced FCB gain for a gain index.
func smplCelpUvGain(gainIdx int16) float32
```

## Implementation suggestions (guidance, not authoritative)

- `SMPL_CELP_MAX_RATES` = 2 (FEC + MAIN); on the 1:1 path only MAIN is populated.
  Reuse the package `smplCelpMaxRates`/`smplCelpIdxMain` already defined for perc.
- Reuse the existing ACB-gain tables (`cbAcbgainsHRQ14`/`cbAcbgainsLRQ14`) and the
  decode-side helpers where the math is identical; keep `f32`/`f64` widths per site.
- The encoder is stateful across subframes (ACB history / zero-input-response). Build
  it once per stream in the analysis front-end, not per subframe.
