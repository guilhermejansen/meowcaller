...
The LPC analysis front-end (encoder side) plus the forward A→NLSF conversion: window
generation, FFT-based autocorrelation, reflection coefficients, monic LPC,
bandwidth expansion, per-subframe interpolation, and the fixed-point `silk_A2NLSF`.
Media layer; turns a windowed analysis buffer into the LPC coefficients and analysis
NLSF that feed the LSF quantizer.

**Validation vector:** `lsf_vectors.json` — pins the LSF parameter path this module
feeds. Copy it verbatim into `mlow/testdata/`. (The reference's own tests in this
...
```rust
//! MLow LPC ANALYSIS FRONT-END (encoder side): faithful port of the C `smpl_window` /
//! `smpl_lpc` / `smpl_bwe_expand` (`smpl_lpc_util.c`, `smpl_lpc.c`) plus `smpl_A2NLSF_16`
//! (`smpl_lsf_wrapper.c` wrapping silk `silk_A2NLSF`). It turns a 20 ms windowed LPC buffer into
//! the post-bandwidth-expansion LPC coefficients `A[0..16]` and their analysis NLSF, which feed the
//! (bit-exact) LSF quantizer.
//!
//! The C derives the autocorrelation NOT by time-domain lags but by an FFT of the windowed signal
//! (512-pt real FFT) → power spectrum → a cosine transform (`brute_dct`) → reflection coefficients
//! (`smpl_ac2rc`) → `A` (`smpl_rc2a`), then bandwidth expansion (`A[i] *= 0.9999^i`). The replacing
//! the old time-domain autocorr + Gaussian lag window (which diverged from the C and produced wrong
//! formants) is the dominant fix for the mute outbound.
//!
//! Float tolerance: the C uses PFFFT; our portable mixed-radix FFT (shared with `smpl_perc`) differs
//! only in FFT-internal rounding, so the resulting `A`/NLSF match the C to a tight float tolerance,
//! not bit-for-bit (documented like the decoder postfilters).
#![allow(clippy::needless_range_loop)]

use super::smpl_perc::rfft_forward_ordered;
...
// ---------- window generation (smpl_lpc.c gen_sin_win / gen_cos_win) ----------
...
/// Port of `smpl_window` for the LPC 20 ms path (`use_lpc_win=true`, `frame_ms=20`, `len=448`).
...
pub(crate) fn smpl_window_lpc20(
    ...
    // mul_vec(in, win1, out, 264)
    for i in 0..SMPL_LPC_WIN1_20MS_LEN {
        out[i] = input[i] * win1[i];
    }
    // memcpy(out+264, in+264, 448-264-64=120)
...
// ---------- brute_dct cosine tables (smpl_lpc.c gen_cos_row) ----------
...
/// Port of `brute_dct`: autocorrelation `R[0..16]` from the power spectrum `F2[0..256]` via the
/// precomputed cosine sums. All accumulation in f64 (the C casts F2 to double first).
...
/// Port of `smpl_ac2rc_dbl`: autocorrelation `R[0..order]` → reflection coefficients (Schur), with
/// `C0[0] *= (1 + reg)`. `rc[k]` is truncated to f32 each step (matching the C).
...
/// Port of `smpl_rc2a`: reflection coefficients → monic LPC `A[0..order]` (`A[0]=1`). All f32.
...
/// Port of `smpl_bwe_expand` (the `bwe>0` path): `A[i] *= bwe^i` for i in 1..=order.
...
/// Number of power-spectrum bins the C `smpl_pitch` / `smpl_get_signal_mode` read (`SMPL_F_LEN`).
...
/// returning the post-BWE monic LPC `A[0..16]` (`A[0]=1`) and the power spectrum `F2[0..256]` (the C
...
pub(crate) fn smpl_lpc_analyze_with_f2(
    ...
    rfft_forward_ordered(&xbuf, &mut f);
    ...
    let tables = build_dct_tables();
    let mut r = [0.0f64; SMPL_LPC_ORDER + 1];
    brute_dct(&tables, &f2d, SMPL_LPC_ORDER, &mut r);
...
/// the C `lsf_interpol_search` may pick when it lowers the residual energy).
...
/// is the decoder's NLSF→A. On a reset (`prev_lsf` all zero) the C copies `lsf` into `prev_lsf`.
...
/// As [`smpl_lpc_interpol`] but for an explicit `lsf_interpol_idx` (the C `smpl_lsf_interpol_4` row).
...
fn silk_smlaww(a32: i32, b32: i32, c32: i32) -> i32 {
    // a32 + ((b32 * (c32 >> 16)) + (((b32 * (c32 & 0xFFFF)) >> 16))) ; the silk_SMLAWW macro =
    // silk_MLA(a32, b32, silk_RSHIFT_ROUND(c32, 16))? No — SMLAWW(a,b,c)=a + SMULWW(b,c),
    // SMULWW(a,b) = silk_SMULWB(a, b>>16) + silk_SMULWB(a, b&0xFFFF) >> 16 ... use the canonical
    // 64-bit form: a32 + ((i64)b32 * c32 >> 16).
...
/// Port of `silk_A2NLSF`: monic whitening coefficients (Q16) → NLSF (Q15, 0..2^15-1). Mutates
/// `a_q16` (bandwidth expansion on non-convergence). `d` is the filter order (even).
...
/// Port of `smpl_A2NLSF_16`: post-BWE float `A[0..16]` (A[0]=1) → analysis NLSF in radians (0..pi).
...
mod tests {
    ...
    // The forward A2NLSF (silk_A2NLSF, fixed-point) must reproduce the C `lsf` from the post-BWE `A`
    // for every captured record. The two share the same fixed-point arithmetic, so this is exact
    // (NLSF in radians within float rounding of the Q15→radians scale).
    #[test]
    fn a2nlsf_matches_c() {
    ...
    // The full front-end (window already applied by the C; we re-window from the raw lpcbuf, then
    // FFT-autocorr → reflection → A → BWE) must reproduce the C post-BWE `A` from the raw lpcbuf.
    // The FFT differs only in internal rounding (portable mixed-radix vs pffft), so A matches to a
    // tight float tolerance (documented, like the decoder postfilters).
    #[test]
    fn front_end_a_matches_c() {
        ...
        // The windowing is exact; the autocorrelation tracks the C FFT/DCT to a tight tolerance.
        assert!(worst_win < 1e-6, "windowing |dwin|={worst_win:.2e}");
    }

    // DIAGNOSTIC: the wire round-trip — feed the C `qi` (grid+stage2) + threaded prev_nlsf to the
    // decoder's NLSF reconstruction and compare to the C `qlsf`. Proves grid/stage2 map directly onto
    // the decoder wire (grid=qi[0], cond centroid=16) and that our decoder rebuilds the same envelope.
    #[test]
    fn decoder_reconstructs_c_qlsf() {
        ...
        for (n, r) in arr.iter().enumerate() {
            ...
            // The decoder reconstruction must match the C qlsf (the envelope the C synthesizes with),
            // proving the grid/stage2 (incl. cond centroid 16) round-trip and the decoder rebuilds the
            // same envelope — so no synthesis-time stabilization port is needed in the decoder.
            let want = fvec(&r["qlsf"]);
            let rd = (0..SMPL_LPC_ORDER)
                .map(|k| (rec[k] - want[k]).abs())
                .fold(0.0f32, f32::max);
            // rec 3 is a near-silence frame whose ill-conditioned A perturbs the threaded prev_nlsf;
            // its envelope is immaterial (silent output). All other frames match to float epsilon.
            if n != 3 {
                assert!(
                    rd < 1e-3,
...
- Float widths are load-bearing: `f32` stays `float32`, `f64` stays `float64`, and the
  reference deliberately truncates intermediate values to `float32` (e.g. `rc[k] = rc_tmp as f32`,
  the per-step `A` updates). Match every f32/f64 boundary exactly or the tolerance
  drifts.
- `SMPL_PI` is the truncated literal `3.1415926535897`, not `math.Pi` — declare it as a
  constant and reuse it for both the f32 window math and the f64 `*_F64` value.
- Trig must use the right precision: window generation calls `sinf`/`cosf` (f32) while
  `gen_cos_row` accumulates `omega` in f64 with a `rem_euclid(2*pi)` after each step.
  Go's `math.Sin`/`math.Cos` are f64; for the f32 window, compute in f64 then cast to
  float32, matching the C `sinf`/`cosf` result as closely as the platform allows.
  TODO(human): decide whether a float32-domain trig helper is needed to hit the
  windowing tolerance the reference asserts (`< 1e-6`).
- `rem_euclid(two_pi)` is a non-negative floating remainder; Go's `math.Mod` can return
  a negative result, so add `two_pi` and `math.Mod` again (or use a positive-remainder
  helper) to match `rem_euclid`.
- The fixed-point `silk_a2nlsf` path is bit-exact: keep every value `int32`, use the
  i64-widened multiply-shift form for `silk_smlaww` and `silk_bwexpander_32`
  (`int32(int64(b)*int64(c) >> 16)`), and use `wrapping_*` semantics (plain `int32`
  arithmetic wraps in Go too). `silk_rshift_round` and the `<<`/`>>` shifts are signed
  arithmetic shifts — Go `>>` on a signed int is arithmetic, so they map directly.
- `nlsf2a` is a caller-supplied closure (the decoder's NLSF→A); model it as a `func`
  parameter. The two interpolation-weight rows are a fixed `[2][4]float32` table.
- The `include!("silk_lsf_cos_tab.rs")` pulls in the 129-entry cosine table; in Go that
  is just the package-level `int32` array (see the mem datasheet) referenced by name.
- Arrays vs slices: the reference uses fixed-size arrays for the hot buffers
  (`[f32; 448]`, `[f32; 17]`). Prefer Go fixed arrays for these so lengths are
  compile-time and copies are cheap; the `nlsf2a` return and the `corr`/`rc` scratch
  can stay slices.
```