...
# Datasheet: `mlow/lsf_quant`

The encoder-side LSF (line spectral frequency) vector quantizer: it turns the
analysis LPC spectrum into the codebook indices written on the wire and the
quantized spectrum the decoder rebuilds. Media layer (codec parameter coding).
...
```rust
//! MLow LSF QUANTIZER (encoder side): faithful port of the C `smpl_lsf_quant_core`
//! (`smpl_lsf_quant.c`). Given the analysis NLSF, it runs the VQ_temp Mahalanobis shortlist over the
//! stage-1 centroids, then an RD beam (`0.5*order*log2(werr)*RDw_adj + bits`) with per-coeff stage-2
//! clamps, returning `qi[0..16]` — the C `lsf_idx`. `qi[0]` is the Rust wire `grid`, `qi[1..16]` the
...
//! tables, qstep, means, reg_cond, min_dist) are dumped verbatim from the C `smpl_get_lsf_CBks()` plus
//! the static `smpl_lsf_tables.c`, so the quantizer math is bit-faithful to the reference.
//!
//! Validated 60/60 bit-exact vs the C `smpl_lsf_quant`/`smpl_lsf_quant_cond` (see the test). It is the
//! dominant fix for the frozen-grid mute and is now wired live in `analysis.rs`, fed the faithful
//! front-end NLSF (`smpl_lpc::smpl_a2nlsf_16`). The earlier "resonant NLSF rings up" regression was a
//! symptom of the WRONG LPC input (the old heuristic front-end), not a missing decoder stabilizer:
//! the decoder's `smpl_reconstruct_nlsf` already reproduces the C `qlsf` (proven in `smpl_lpc`'s
//! `decoder_reconstructs_c_qlsf`), so no synthesis-time stabilization port was needed.
...
struct St1Json {
    cbhalf: Vec<Vec<f32>>, // [16][16]
    #[serde(rename = "cInv")]
    c_inv: Vec<Vec<f32>>, // [16][16]
    bits_cond: Vec<f32>,   // [17]
    #[serde(rename = "Rotcond")]
    rotcond: Vec<Vec<Vec<f32>>>, // [2][16][16]
    #[serde(rename = "cbCinv")]
    cb_cinv: Vec<Vec<f32>>, // [16][16]
    we: Vec<Vec<Vec<f32>>>, // [16][16][16]
    bits: Vec<f32>,        // [16]
    wie: Vec<Vec<Vec<f32>>>, // [16][16][16]
}

#[derive(serde::Deserialize)]
struct St2Json {
    #[serde(rename = "numQlvls")]
    num_qlvls: Vec<i32>, // [16]
    #[serde(rename = "Qlvls")]
    qlvls: Vec<Vec<f32>>, // [16][numQlvls[i]]
    #[serde(rename = "numBits")]
    num_bits: Vec<Vec<f32>>, // [16][numQlvls[i]]
}

#[derive(serde::Deserialize)]
struct LsfCbJson {
    st1: Vec<St1Json>,               // [2]
    st2: Vec<Vec<Vec<St2Json>>>,     // [2][2][17]
    min_qi: Vec<Vec<Vec<Vec<i32>>>>, // [2][2][17][16]
    max_qi: Vec<Vec<Vec<Vec<i32>>>>, // [2][2][17][16]
    qstep: Vec<Vec<f32>>,            // [2][2]
    mean_v: Vec<f32>,                // [16]
    mean_uv: Vec<f32>,               // [16]
    reg_cond: Vec<f32>,              // [2]
    min_dist_v: Vec<f32>,            // [17]
    min_dist_uv: Vec<f32>,           // [17]
}
...
static LSF_CB: OnceLock<LsfCb> = OnceLock::new();
...
/// `smpl_matrix_mult_transp_16(C, x, y, 16, len_x)`: `y[i] = sum_j C[j][i]*x[j]`.
...
/// A faithful equivalent of `smpl_get_maxi_K` for the VQ shortlist + alt-refinement purpose (the
/// caller re-evaluates survivors, so the survivor SET is the load-bearing output).
...
/// Port of `smpl_lsf_weights` (the `SMPL_USE_SPEC_LSW_WEIGHT` path, which the reference build enables):
/// the RD weight at each LSF is the inverse spectral envelope magnitude `1/sqrt(|A(e^jw)|^2 * scale)`,
...
/// Port of `SMPL_lsf_min_dist`: push LSFs apart so consecutive spacings exceed `min_dist`.
fn lsf_min_dist(lsfs: &mut [f32], min_dist: &[f32]) {
    ...
    // C asserts here; in our pipeline we fall through with the best-effort spacing.
}
...
/// Faithful port of `smpl_lsf_quant_core`. `nlsf` is the analysis NLSF (`smpl_A2NLSF_16(A)`), `voiced`
/// 0/1 selects the codebook, `low_rate` 0/1, `cond` the conditional-coding params (Some => qi[0]==16
/// reachable), `rd_w_adj` the RD weight. `surv` is the beam width (= lsf_surv).
fn lsf_quant_core(
    cb: &LsfCb,
    a: &[f32],
    nlsf: &[f32],
    ...
    // RD weights are the spectral inverse-envelope (reference build defines SMPL_USE_SPEC_LSW_WEIGHT).
    ...
    for s1 in 0..surv {
        ...
        // The clamp tables are indexed by qi1 (0..16); for the cond centroid the C uses the qi1==16 row.
        ...
        let bits_orig = bits;
        // The beam base `lsfq_` is FIXED to the initial lsfq (C `memcpy(lsfq_, lsfq, ...)` BEFORE the
        // loop); each refinement flips ONE coeff relative to this base, undoing the previous flip.
        let lsfq_base = lsfq;
...
/// `smpl_lsf_quant`: non-conditional LSF quantization. `a` is the monic LPC `A[0..16]` (A[0]=1).
...
/// `smpl_lsf_quant_cond`: conditional LSF quantization given the previous frame's quantized NLSF.
/// `a` is the monic LPC `A[0..16]` (A[0]=1).
...
/// Port of `smpl_rot_apply_wght`: builds `wrot1` (=we) and `wrot2` (=wie) for the cond centroid from
/// the rotation matrix and the Laroia-weighted previous LSF.
...
mod tests {
    ...
    // Tier-B: feed the C's post-A2NLSF `lsf` + LPC `A` + state to the Rust port; both the indices
    // `qi[]` AND the reconstructed `qlsf` must match the C reference bit-for-bit.
    #[test]
    fn lsf_quant_matches_c() {
        ...
        assert_eq!(bad, 0, "lsf_quant qi mismatch vs C");
    }
}
```
...
// LsfQuantResult is one LSF quantization: Qi[0] (=grid), Qi[1..16] (=stage2),
// and the reconstructed quantized NLSF.
type LsfQuantResult struct {
	Qi   [SmplLPCOrder + 1]int32
	QLsf [SmplLPCOrder]float32
}

// LsfCb holds the loaded LSF codebook tables.
type LsfCb struct {
	// ...
}
...
// LsfQuant is the non-conditional LSF quantization. a is the monic LPC A[0..16] (A[0]=1).
...
- All math is `f32`; map to Go `float32` throughout and do not promote to `float64`
  in intermediate accumulations, since the indices `qi[]` are decided by float
  comparisons and any widening can flip a tie.
- `usize` selector args (`voiced`, `low_rate`, `surv`) → Go `int`; the on-wire
  `qi[]` values are `i32` → Go `int32`.
- Fixed-length vectors of width 16 can be Go arrays `[16]float32` for the per-coeff
  state; the codebook tables are ragged (per-coeff `Qlvls`/`numBits` lengths vary)
  so those stay slices.
